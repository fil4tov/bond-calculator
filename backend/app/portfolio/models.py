...
class Bond(Base):
    ...
    purchases: Mapped[list["BondPurchase"]] = relationship(back_populates="bond", cascade="all, delete-orphan")
...
class BondPurchase(Base):
    ...
    __table_args__ = (
        CheckConstraint("amount_spent > 0", name="ck_bond_purchases_amount_spent_positive"), CheckConstraint("quantity > 0", name="ck_bond_purchases_quantity_positive"),
        ForeignKeyConstraint(["bond_id", "user_id"], ["bonds.id", "bonds.user_id"], ondelete="CASCADE", name="fk_bond_purchases_bond_owner"),
    )
