...
def bond_data(name: str) -> BondCreate:
    ...
    return BondCreate.model_validate({"instrument_uid": "uid", "ticker": "TICK", "name": name, "nominal": "1000.00", "payments_per_year": 2, "placement_date": (today - timedelta(days=365)).isoformat(), "maturity_date": (today + timedelta(days=365)).isoformat(), "amount_spent": "900.00", "quantity": 1, "purchase_date": (today - timedelta(days=1)).isoformat()})
...
async def test_cascade_deletes_schedule_at_orm_level(client: AsyncClient, session_factory: async_sessionmaker[AsyncSession]) -> None:
    ...
    async with session_factory() as session:
        user = (await session.scalars(select(User))).one()
        bond = Bond(user_id=user.id, instrument_uid="uid", ticker="T", name="Cascade", nominal=Decimal("1"), payments_per_year=0, placement_date=date(2026, 1, 1), maturity_date=date(2027, 1, 1))
        bond.coupon_schedules.append(BondCouponSchedule(figi="F", coupon_date=date(2026, 6, 1), coupon_number=1, fix_date=None, pay_one_bond_amount=Decimal("1"), pay_one_bond_currency="RUB", coupon_type=1, coupon_start_date=date(2026, 1, 1), coupon_end_date=date(2026, 6, 1), coupon_period=1))
        session.add(bond); await session.commit(); await session.delete(bond); await session.commit()
...
async def test_database_rejects_invalid_schedule_constraints(client: AsyncClient, session_factory: async_sessionmaker[AsyncSession]) -> None:
    ...
    async with session_factory() as session:
        user = (await session.scalars(select(User))).one()
        bond = Bond(user_id=user.id, instrument_uid="uid", ticker="T", name="Schedule", nominal=Decimal("1"), payments_per_year=0, placement_date=date(2026, 1, 1), maturity_date=date(2027, 1, 1))
        bond.coupon_schedules.append(BondCouponSchedule(figi="F", coupon_date=date(2026, 6, 1), coupon_number=1, fix_date=None, pay_one_bond_amount=Decimal("-1"), pay_one_bond_currency="RUB", coupon_type=1, coupon_start_date=date(2026, 6, 2), coupon_end_date=date(2026, 6, 1), coupon_period=-1))
        session.add(bond)
        with pytest.raises(IntegrityError):
            await session.commit()
