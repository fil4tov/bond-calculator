...
- Produces: An isolated temporary-database integration contract requiring revision `20260813_0001`, exactly the five current application tables, current columns/indexes/check constraints, zero ORM drift, and a working downgrade/re-upgrade cycle without interfering with shared test fixtures.
...
The first test catches a wrong revision, missing/current table, column, index, check constraint, or ORM drift. The second catches unsafe dependency order in `downgrade()`. Each test creates and force-drops its own uniquely named PostgreSQL database so existing fixtures cannot invalidate Alembic state.
