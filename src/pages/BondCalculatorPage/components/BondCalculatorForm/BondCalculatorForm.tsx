...
import { Button, IconButton, NumberField, SegmentedControl, TextField, Typography } from '#shared/ui';
...
interface NumberControllerProps<T extends FieldValues> extends Omit<InputHTMLAttributes<HTMLInputElement>, 'name' | 'value' | 'defaultValue'> {
  control: Control<T>;
  name: FieldPath<T>;
  label: ReactNode;
  hint?: ReactNode;
...
function ControlledNumberField<T extends FieldValues>({ control, name, label, hint, unit, integer, wide, error, ...props }: NumberControllerProps<T>) {
  return (
    <Controller
      control={control}
      name={name}
      render={({ field }) => (
        <NumberField
          ...
          onFocus={(event) => {
            const value = parseFormattedNumber(event.currentTarget.value);
            field.onChange(formatEditableNumber(value));
            props.onFocus?.(event);
          }}
          onBlur={(event) => {
